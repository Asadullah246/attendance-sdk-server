 ...
 * Monthly/range summary per employee
 */
router.get('/summary', asyncHandler(async (req: Request, res: Response) => {
  const { employeeId, dateFrom, dateTo } = req.query;

  if (!employeeId) {
    return res.status(400).json(errorResponse('employeeId is required for summary', 400));
  }
...
router.post('/calculate', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!date) {
    return res.status(400).json(errorResponse('date is required (YYYY-MM-DD)', 400));
  }
...
/**
 * PUT /api/v1/reports/:id/override
 * HR manual override
 */
router.put('/:id/override', asyncHandler(async (req: Request, res: Response) => {
